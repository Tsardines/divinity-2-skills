...
function createComponentProp(isRequired) {
  ...
  return function componentPropType(props, key, componentName, location, propFullName) {
    ...
    if (prop == null) {
      if (isRequired) {
        message = `The ${location} \`${propName}\` is marked as required in \`${componentName}\`, ` + `but its value is \`${typeof prop}\`.`;
      }
    } else if (!ReactIs.isValidElementType(prop)) {
      const preciseType = typeof prop;
      message = `Invalid ${location} \`${propName}\` of type \`${preciseType}\` ` + `supplied to \`${componentName}\`, expected a component.`;
    }
